...
public class MainWrapper {
  ...
  /*  total = total + score[1];
    ...
    if(max < score[1]) { max = score[2]; top = 2;}
    if(max < score[1]) { max = score[3]; top = 3;}
    if(max < score[1]) { max = score[4]; top = 4;}
    
    if(min > score[1]) { min = score[1]; bottom = 1;}
    if(min > score[1]) { min = score[2]; bottom = 2;}
    if(min > score[1]) { min = score[3]; bottom = 3;}
    if(min > score[1]) { min = score[4]; bottom = 4;}
