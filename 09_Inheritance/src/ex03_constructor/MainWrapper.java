...
/*
 * 상속 관계의 클래스들의 생성자 
 * 1. 자식 클래스는 생성할 때 "반드시" 부모 클래스를 "먼저"생성해야 한다. 
 * 2. 서브 클래스는 생성할 때 "반드시" 슈퍼 클래스를 "먼저"생성해야 한다. 
 * 3. 서브 클래스의 생성자가 호출될 때는 "반드시" 슈퍼 클래스의 생성자를 "먼저" 호출해야 한다.
 * 4. 만약 서브 클래스의 생성자에서 슈퍼클랫를 호출하지 않으면 java에 의해서 슈퍼 클래스의 "디폴트 생성자"가 호출된다. 
 * 5. 서브클래스의 생성자에서 슈퍼 클래스의 생성자를 호출하는 방법은 super()이다.  
 */
