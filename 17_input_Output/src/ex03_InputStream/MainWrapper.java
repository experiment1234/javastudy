...
public class MainWrapper {
  ...
  public static void ex01() {
    ...
    try {
      ...
      //파일 입력 스트림 생성(반드시 예외 처리 필요, 파일이 없으면 예외 발생함) 
  ...
  public static void ex02() {
    ...
    try {
      ...
      while( (readByte = fin.read(b)) != -1) {
       sb.append(new String(b, 0, readByte)); //배열 b의 인덱스 0부터 readByte개 데이터를 String으로 반환한다.ㄴ
      }
  ...
  //바이트 기반 스트림은 원래 한글처리가 안된다. 
   ...
   BufferedInputStream bin = null;
    ...
    try {
      ...
      //버퍼 입력 스트림 생성(반드시 예외 처리 필요, 파일이 없으면 예외 발생함) 
   ...
   */
    ...
    try {
      ...
      //데이터 입력 스트림 생성(반드시 예외 처리 필요, 파일이 없으면 예외 발생함) 
