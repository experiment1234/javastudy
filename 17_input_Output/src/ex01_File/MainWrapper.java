...
public class MainWrapper {
  ...
  /*
    ...
    */
     ...
     * 2.리눅스 : 슬래시(/)
     * 3.java.io.File 클래스에는 플랫폼에 따라서 경로 구분자를 자동으로 바꿔주는 separator 필드값이 있다. 
     * 
     */
    public static void ex01() {
      ...
      //File 객체 생성
      dir = new File("C:" + File.separator + "storage"); // c드라이브 아래에 있는 storage디렉터리
    ...
    public static void ex02() {
      ...
      if(file.exists()) {
        ...
        file.createNewFile();// 반드시 예외 처리를 해야하는 코드(Checked Exception인 IOException 발생)
    ...
    public static void ex03() {
      ...
      // 파일, 디렉터리 정보 확인
      // listFiles()    : 모든 File객체 저정한 File[] 반환
      // getName()      : 이름 반환
      // getParent()    : 저장된 디렉터리 반환
      // getPath()      : getParent() + getName()
      // lastModified() : 최종수정일을 long 타입으로 반환
      // length()       : 크기를 long타입의 바이트 단위로 반환
      // isDirectory()  : 디렉터리이면 true반환
      // isFile()        : 파일이면 true반환
      ...
      // 디렉터리에 있는 모든 file객체(파일, 디렉터리) 가져오기
      File[] files = dir.listFiles();
      
      // 디렉터리에 있는 모든 file객체의 정보 확인하기
      for(int i = 0; i < files.length; i++) {
        ...
        //파일은 바이트 단위로 크기가 저장, 디렉터리는 크기가 표시하지 않으므로 0으로 저장(
